function varargout=dimerrf(B,E,omega)
alp=1;
eta=40/6;
n1=500;
n2=500;
...
psiLp=R_kp_up./(E+Eth-eta*Kmod_square-(Kmod_square-2*B+2*sqrt(alp^2*ndK_perp.^2+B^2)));
psiLm=R_km_up./(E+Eth-eta*Kmod_square-(Kmod_square-2*B-2*sqrt(alp^2*ndK_perp.^2+B^2)));


denomip=omega-(E+Eth-eta*Kmod_square-(Kmod_square-2*B+2*sqrt(alp^2*ndK_perp.^2+B^2)))+zeroi;
denomim=omega-(E+Eth-eta*Kmod_square-(Kmod_square-2*B-2*sqrt(alp^2*ndK_perp.^2+B^2)))+zeroi;

part1=-imag(sum(sum(ndK_perp.*WK_perp.*WK_z.*psiRp.*psiLp./denomip)));
part2=-imag(sum(sum(ndK_perp.*WK_perp.*WK_z.*psiRm.*psiLm./denomim)));
varargout{1}=part1+part2;%总的
varargout{2}=part1;%+的部分
varargout{3}=part2;%-的部分
varargout{4}=abs(sum(sum(sum(ndK_perp.*WK_perp.*WK_z.*psiRm))));
varargout{5}=abs(sum(sum(sum(ndK_perp.*WK_perp.*WK_z.*psiRp))));
end
